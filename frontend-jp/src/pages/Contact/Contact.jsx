    ...
    return (
        ...
        >
            ...
            <form className="mt-[40px] space-y-[24px]">
                ...
                <div className="flex flex-col gap-3 pb-4 lg:pb-6">
                    <ContactLabel label="知ったきっかけ" name="known" require />
                    <div className={`${error.known ? 'border-[1px] border-[#E32222CC] rounded-[8px] px-[15px] py-[6px]' : ''} space-y-2 lg:space-y-0`}>
                        <ContactRadio state={formData.known} setState={(value) => setFormData(prev => ({ ...prev, known: value }))} error={error.known || []} label="ご紹介" name="introduction" value="1" />
                        <ContactRadio state={formData.known} setState={(value) => setFormData(prev => ({ ...prev, known: value }))} error={error.known || []} label="SNS" name="sns" value="2" />
                        <ContactRadio state={formData.known} setState={(value) => setFormData(prev => ({ ...prev, known: value }))} error={error.known || []} label="その他媒体" name="other" value="3" />
                ...
                {(!!loading || !check) && (
                    <button className="text-[16px] lg:text-[20px] font-bold leading-[22px] tracking-[-2%] text-white w-full rounded-[40px] py-5 bg-[#b87768] cursor-not-allowed" disabled>
                        {!!loading && (
                            <>
                ...
                {!loading && check && (
                    <ContactButton buttonFunc={validate}>確認する</ContactButton>
                )}
            </form>
        </ContactFrame>
    );
