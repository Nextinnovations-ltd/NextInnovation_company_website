...
const Gif = () => {
  ...
  return (
    <div className="gif-section bg-cover bg-center w-full h-screen bg-no-repeat relative">
      <div className="rotate absolute">
          <img src={NiLoop} alt="" />
      </div>
        <div 
        ref={buttonRef}
        data-magnetic-value="0.5" // Increased default strength
        className="magnetic-button consult-free cursor-default flex gap-3 md:gap-x-4 lg:gap-x-6 xl:gap-x-8 justify-center items-center absolute"
      >
        <img
          ref={addContentRef}
          src={Hand}
          alt="Hand pointing"
          className="w-[48px] md:w-[64px] lg:w-[70px] xl:w-[85px]"
        />
        <Link to="/contact"
          ref={addContentRef}
          className="text-[28px] text-nowrap md:text-[35px] lg:text-[40px] xl:text-[56px] uppercase font-semibold"
        >
          consult free
        </Link>
      </div>
