...
const Navbar = () => {
	...
	const servicesActivePaths = ['/eor', '/dx-support', '/development', '/uiux'];
	const aboutActivePaths = ['/company-profile', '/business-philosophy', '/ceo-message'];
		...
		<>
			...
			>
				...
				<div className="nav-menu md:w-[700px] lg:w-[835px] w-[835px] h-[100%] hidden lg:flex justify-between items-center text-[16px]  leading-[100%] tracking-[-4%]">
					...
					>
						<button
							className={`flex gap-[6px] items-center py-2 ${servicesActivePaths.includes(location.pathname) ? 'border-b-2 border-[#F15A29]' : ''}`}
						>
							<h4>サービス</h4>
							<DownArrow />
						</button>
					...
					>
						<button
							className={`flex gap-[6px] items-center py-2 ${aboutActivePaths.includes(location.pathname) ? 'border-b-2 border-[#F15A29]' : ''}`}
						>
							<h4>会社情報</h4>
							<DownArrow />
						</button>
						...
						>
							<DropDownModal
								...
								menus={[
									...
									{ title: "経営方針", link: "/business-philosophy" },
